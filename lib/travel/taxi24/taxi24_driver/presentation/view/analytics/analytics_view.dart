...
class _AnalyticsView extends ConsumerState<AnalyticsView>{
  ...
  List<String> timers = ["Daily", "Weekly", "Monthly"];
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            ...
                            SizedBox(width: 10,),
                            (selectedIndex==0 && DateTime.now().toLocal().day > DateTime.parse(data).toLocal().day )||
                                (selectedIndex==1 &&DateTime.now().toLocal().month > DateTime.parse(data).toLocal().month)||
                                ( selectedIndex==2 &&DateTime.now().toLocal().weekday > DateTime.parse(data).toLocal().weekday)?
                            GestureDetector(
                              behavior: HitTestBehavior.translucent,
                              onTap: (){
                        ...
                        AspectRatio(
                            aspectRatio: 2.5,
                            child: Chart(
                              title: viewModel.analyticsEntity!.analysis!.map((e) => e.key!.toString().length>3?e.key!.substring(0,3):e.key!).toList(),
                              values: viewModel.analyticsEntity!.analysis!.map((e) => e.value).toList(),
                              multivalued:viewModel.selectedIndex == 2 ? true :false ,

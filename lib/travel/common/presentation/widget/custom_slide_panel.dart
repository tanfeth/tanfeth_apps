...
import 'package:tanfeth_apps/travel/common/presentation/widget/navigation_to_map.dart';
...
class CustomSlidePanel extends StatefulWidget {
  ...
  final double promoMinHeight;
  final double maxHeight;
  final double promoMaxHeight;
  ...
  final bool locationDetectorEnabled;
  ...
  CustomSlidePanel(
      ...
      this.collapsed,
      this.minHeight = 200,
      this.promoMinHeight = 82,
      this.maxHeight = 600.0,
      this.promoMaxHeight = 700.0,
      this.snapPoint,
      this.border,
      this.navigationTap,
      this.borderRadius,
      ...
      this.locationDetectorEnabled = false,
      ...
      this.header,
      this.footer})
      : assert(child != null || panelBuilder != null),
        assert(0 <= backdropOpacity && backdropOpacity <= 1.0),
        assert(snapPoint == null || 0 < snapPoint && snapPoint < 1.0),
        super(key: key);

  @override
  _CustomSlidePanelState createState() => _CustomSlidePanelState();
}

class _CustomSlidePanelState extends State<CustomSlidePanel>
      ...
      children: <Widget>[
        ...
        !_isPanelVisible
            ...
            : Column(
                ...
                children: [
                  widget.navigationEnabled == true ? Row(
                    children:  [const Spacer(), NavigationToMap(navigationTap:widget.navigationTap??(){})],
                  ) : Container(),
                  widget.locationDetectorEnabled == true ? Row(children:  [const Spacer(), 
                    CurrentLocationDetector(onTap:widget.onTapCurrentLocation??(){})],) 
                      : SizedBox.shrink(),
                  Container(
                    decoration: BoxDecoration(
                      ...
                      borderRadius:widget.hasBorderRadius? BorderRadius.only(
                    ...
                    child: Column(
                      children: [
                        if (widget.hasHeader)
                          _gestureHandler(
                            child: Container(
                              height: _ac.value * (widget.promoMaxHeight - widget.promoMinHeight) + widget.promoMinHeight,
                              decoration: BoxDecoration(
                                gradient:  LinearGradient(colors: [
                                  context.color.primary,
                                  context.color.primary.withOpacity(0.3),
                            ...
                            child: Stack(
                              children: <Widget>[
                                ...
                                Positioned(
                                    ...
                                    child: Stack(
                                      children: [
                                        // if(widget.hasPromoCode) const CustomSlideInformationPanel(child:  Center(),),
                                        Container(
...
class PanelController {
  _CustomSlidePanelState? _panelState;

  void _addState(_CustomSlidePanelState panelState) {
    _panelState = panelState;
  }

  /// Determine if the panelController is attached to an instance
  /// of the CustomSlidePanel (this property must return true before any other
  ...
  Future<void> close() {
    assert(
        isAttached, "PanelController must be attached to a CustomSlidePanel");
  ...
  /// Requires that the CustomSlidePanel snapPoint property is not null
  /// (optional) duration specifies the time for the animation to complete
  /// (optional) curve specifies the easing behavior of the animation.
  Future<void> animatePanelToSnapPoint(
    ...
    assert(_panelState!.widget.snapPoint != null,
        "CustomSlidePanel snapPoint property must not be null");
    return _panelState!
